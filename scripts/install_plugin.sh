...
if [ -x "$(which curl 2>/dev/null)" ]; then
    ...
    curl -sSL "${url}.prov" -o "${HELM_PLUGIN_DIR}/releases/v${version}.tgt.prov"
else
    wget -q "${url}" -O "${HELM_PLUGIN_DIR}/releases/v${version}.tgz"
    wget -q "${url}.prov" -O "${HELM_PLUGIN_DIR}/releases/v${version}.tgz.prov"
fi
tar xzf "${HELM_PLUGIN_DIR}/releases/v${version}.tgz" -C "${HELM_PLUGIN_DIR}/releases/v${version}"
